...
export class shop extends Component {
    ...
    onBtnGetClick() {
        ...
        if (playerData.instance.hasCar(carID)) {
            ...
            if (this.currentCarInfo.num > ethersHelper.instance.datas.gold) {
                //金币不足
                // return;
                uiManager.instance.showTips(/*i18n.t("shop.getGold")*/'Insufficient coins', () => { });
                return;
            }
            ...
            ethersHelper.instance.buyCar(carID, this.currentCarInfo.num).then(() => {
                ...
                uiManager.instance.showDialog('common/showReward', [rewardInfo, false, i18n.t("showReward.buySuccessful"), () => {
                    //启用
                    gameLogic.useCar(carID);
                    this.currentPage!.getComponent(shopPage)!.refreshUse(carID);
                }, null, i18n.t("showReward.confirm")]);
                uiManager.instance.hideDialog('common/connetingWallet');
                return ethersHelper.instance.queryGold()
            }).then((balanceOf) => {
                clientEvent.dispatchEvent('updateGold');
            }).catch((err) => {
                uiManager.instance.hideDialog('common/connetingWallet');
                console.error("onBtnGetClick err", err)
            })

            return

            const signerAddress = playerData.instance.web3ProviderData.signerAddress
            const erc20 = playerData.instance.web3ProviderData.erc20
            const carNft = playerData.instance.web3ProviderData.carNft
            erc20.approve(playerData.instance.web3ProviderData.carNftAdress, ethers.constants.MaxUint256)
                .then(() => {
                    return carNft.buyCar()
                }).then((buycar) => {
                    console.info(buycar, "buycar")
                    return buycar.wait()
                }).then((wait) => {
                    console.info("wait", wait)
                    return carNft.tokenOfOwnerByIndex(signerAddress, playerData.instance.web3ProviderData.cars.length)
                })
                .then((carId: BigNumber) => {
                    const newCardId = carId.toNumber()

                    playerData.instance.web3ProviderData.cars.push(newCardId)
                    const id = newCardId % localConfig.instance.getCars().length
                    ...
                    uiManager.instance.showDialog('common/showReward', [rewardInfo, false, i18n.t("showReward.buySuccessful"), () => {
                        //启用
                        gameLogic.useCar(id);
                        this.currentPage!.getComponent(shopPage)!.refreshUse(id);
                    }, null, i18n.t("showReward.confirm")]);

                    return erc20.balanceOf(signerAddress)
                })
                .then((balanceOf) => {
                    console.info("buycar erc20 balanceOf", balanceOf)
                    playerData.instance.web3ProviderData.gold = ethers.utils.formatEther(balanceOf)
                    clientEvent.dispatchEvent('updateGold');
                }).catch((err) => {
                    console.error("onBtnGetClick", err)
                })
    ...
    updateButtons() {
        if (playerData.instance.hasCar(this.currentCarInfo.ID)) {
            ...
            this.lbGo.string = i18n.t('shop.use');
            ...
            if (ethersHelper.instance.datas.gold >= this.currentCarInfo.num) {
