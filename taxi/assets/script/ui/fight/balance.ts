...
import { ethers } from "../../../lib/ethers/ethers-5.2.esm.min.js";
...
export class balance extends Component {
    ...
    show(level: number, curProgress: number, isTakeOver: boolean, maxProgress: number, money: number, isFinishLevel: boolean) {
        ...
        //触发显示
        this.nodeDouble.active = false//percent < 30;
    }
    ...
    private afterRewardOver(rewardMoney: number) {
        //如果关卡是完成的，进入下一关
        //如果关卡是未完成的，还是保留同一关
    ...
    rewardOver(rewardMoney: number) {
        if (rewardMoney > 0) {
            console.log("rewardMoney", rewardMoney)
            uiManager.instance.showDialog('common/connetingWallet');
            ethersHelper.instance.rewardOver(rewardMoney)
                .then(() => {
                    uiManager.instance.hideDialog('common/connetingWallet');
                    this.afterRewardOver(rewardMoney)
                    ...
                    uiManager.instance.hideDialog('common/connetingWallet');
                    console.error("rewardOver err", err)
                });
        } else {
            this.afterRewardOver(rewardMoney)
        }
